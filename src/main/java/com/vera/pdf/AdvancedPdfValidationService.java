...
@Service
public class AdvancedPdfValidationService {
	...
	private static final Path ARIAL_REGULAR_FONT = Paths.get("C:\\Windows\\Fonts\\arial.ttf");
	private static final Path ARIAL_BOLD_FONT = Paths.get("C:\\Windows\\Fonts\\arialbd.ttf");

	private final PdfValidationProperties properties;
	private final ObjectMapper objectMapper = new ObjectMapper();

	public AdvancedPdfValidationService(PdfValidationProperties properties) {
	...
	private void writeStreamAsSingleArtifact(List<Object> tokens, ContentStreamWriter writer, PDResources resources)
		...
		int artifactDepth = 0;
		boolean plainArtifactOpen = false;
		...
		for (Object token : tokens) {
			...
			if ("BMC".equals(operatorName) || "BDC".equals(operatorName)) {
				...
				if (plainArtifactOpen && !suppressedMarkedContent) {
					writer.writeToken(Operator.getOperator("EMC"));
					plainArtifactOpen = false;
				}
				...
				if (artifactDepth == 0 && !plainArtifactOpen && !hasTaggedAncestor(markedContentStack)) {
					writer.writeToken(COSName.ARTIFACT);
					writer.writeToken(Operator.getOperator("BMC"));
					plainArtifactOpen = true;
				}
				writeOperatorGroup(writer, operands, operator);
				operands.clear();
	...
	private void fixFontAndTextIssues(PDDocument document) throws IOException {
		Map<Path, PDFont> embeddedSubstitutes = new HashMap<>();
		for (PDPage page : document.getPages()) {
			PDResources resources = page.getResources();
			if (resources != null) {
				scanFonts(document, resources, new HashSet<>(), embeddedSubstitutes);
			}
		}
	}
	...
	private void scanFonts(PDDocument document, PDResources resources, Set<COSBase> visitedResources,
			Map<Path, PDFont> embeddedSubstitutes) throws IOException {
		if (!visitedResources.add(resources.getCOSObject())) {
			return;
		}
		for (COSName fontName : resources.getFontNames()) {
			...
			if (font != null) {
				PDFont embeddedSubstitute = embeddedHelveticaSubstitute(document, font, embeddedSubstitutes);
				if (embeddedSubstitute != null) {
					resources.put(fontName, embeddedSubstitute);
				}
				else {
		...
		scanNestedFormResources(document, resources, visitedResources, embeddedSubstitutes);
	}

	private PDFont embeddedHelveticaSubstitute(PDDocument document, PDFont font, Map<Path, PDFont> embeddedSubstitutes)
			throws IOException {
		if (font.isEmbedded() || font.getSubType() == null || !font.getSubType().equals("Type1")) {
			return null;
		}
		Path substitutePath = switch (font.getName()) {
			case "Helvetica", "Helvetica-Oblique" -> ARIAL_REGULAR_FONT;
			case "Helvetica-Bold", "Helvetica-BoldOblique" -> ARIAL_BOLD_FONT;
			default -> null;
		};
		if (substitutePath == null || !Files.isRegularFile(substitutePath)) {
			return null;
		}
		Path normalizedPath = substitutePath.toAbsolutePath().normalize();
		return embeddedSubstitutes.computeIfAbsent(normalizedPath, path -> loadWinAnsiTrueTypeFont(document, path));
	}

	private PDFont loadWinAnsiTrueTypeFont(PDDocument document, Path fontPath) {
		try {
			File fontFile = fontPath.toFile();
			return PDTrueTypeFont.load(document, fontFile, WinAnsiEncoding.INSTANCE);
		}
		catch (IOException ex) {
			throw new PdfValidationException("Unable to embed substitute font " + fontPath + ".", ex);
		}
	}
	...
	private void scanNestedFormResources(PDDocument document, PDResources resources, Set<COSBase> visitedResources,
			Map<Path, PDFont> embeddedSubstitutes) throws IOException {
		for (COSName name : resources.getXObjectNames()) {
			PDXObject xObject = resources.getXObject(name);
			if (xObject instanceof PDFormXObject form && form.getResources() != null) {
				scanFonts(document, form.getResources(), visitedResources, embeddedSubstitutes);
			}
		}
	}
